...
Runs on Windows, macOS, and Linux with Python 3.8+. A KCD .pak is a plain ZIP, but CryPak rejects
the per-file modification-time metadata that GUI zip tools add, so
...
def read_manifest_tag(manifest_path: str, tag: str) -> str:
    """Return the first <tag>value</tag> from the manifest, or '' if absent.

    XML comments are stripped first so a commented-out tag is never mistaken for the real value.
    """
...
def main() -> None:
    ...
    # argparse validates --compress, but the value can also arrive as the env-sourced default,
    # which bypasses choices=, so re-check it here.
    if arguments.compress not in ("deflate", "store"):
        fail(f"invalid compress mode: '{arguments.compress}' (use 'deflate' or 'store')")
    ...
    if github_output:
        with open(github_output, "a", encoding="utf-8") as handle:
            ...
            handle.write(f"deploy-dir={deploy_dir}\n")
