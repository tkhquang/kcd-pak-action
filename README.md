...
Run `actions/checkout` before this action so `mod-dir` exists in the workspace. The runner needs
Python 3.8+ (present on GitHub-hosted runners; install it on self-hosted ones).
...
| `deploy-dir` | `dist/loot_beacon`            | Deploy-ready mod folder. |
...
You can build the `.pak` files on your own computer. You need **Python 3.8 or newer**.
...
```text
KCD2:  ...\steamapps\common\KingdomComeDeliverance2\Mods\
KCD1:  ...\steamapps\common\KingdomComeDeliverance\Mods\
```
